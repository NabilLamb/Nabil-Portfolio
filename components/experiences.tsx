  ...
  return (
    <section id="experiences" className="relative py-24 md:py-32 px-4 sm:px-6 lg:px-8 overflow-hidden" ref={ref}>
      <canvas
        ...
        style={{ background: "linear-gradient(135deg, #0a0a14 0%, #0f0f1a 100%)" }}
      />
      ...
      <div className="absolute inset-0 -z-10 opacity-5">
        <div className="absolute inset-0" style={{ backgroundImage: `linear-gradient(to right, rgba(120, 119, 198, 0.1) 1px, transparent 1px), linear-gradient(to bottom, rgba(120, 119, 198, 0.1) 1px, transparent 1px)`, backgroundSize: "40px 40px" }} />
      </div>
      ...
      <div className="max-w-7xl mx-auto relative">
        <div className={`transition-all duration-1000 mb-16 text-center ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
          ...
          <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
            A timeline of my professional growth in the tech industry
          </p>
        </div>
        ...
        <div className="hidden lg:block relative">
          ...
          <div className="absolute left-1/2 top-0 bottom-0 -translate-x-1/2">
            {data.map((_, idx) => (
              <div key={idx} className="absolute left-1/2 -translate-x-1/2 w-4 h-4 rounded-full bg-gradient-to-r from-purple-500 to-cyan-500 border-2 border-background" style={{ top: `${(idx / (data.length - 1)) * 100}%`, transform: "translate(-50%, -50%)" }}>
            ...
            {data.map((exp, idx) => (
              <div key={idx} className={`relative transition-all duration-1000 ${isVisible ? "opacity-100" : "opacity-0"}`} style={{ marginLeft: idx % 2 === 0 ? "0" : "auto", width: "45%" }} onMouseEnter={() => setActiveIndex(idx)}>
                <div className={`relative p-6 rounded-2xl border backdrop-blur-sm transition-all duration-300 hover:scale-[1.02] group ${idx % 2 === 0 ? "mr-auto ml-0" : "ml-auto mr-0"} ${activeIndex === idx ? "border-cyan-500/50 bg-gradient-to-r from-purple-500/10 via-background/50 to-cyan-500/10 shadow-xl shadow-cyan-500/10" : "border-white/10 bg-white/5 hover:border-purple-500/30"}`}>
                  ...
                  <p className="text-muted-foreground mb-6 leading-relaxed group-hover:text-white/80 transition-colors duration-300">{exp.description}</p>
        ...
        <div className={`mt-16 transition-all duration-1000 delay-800 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: "Years Coding", value: "2+", icon: <Clock size={20} />, color: "text-purple-400" },
              { label: "Projects Built", value: "6+", icon: <Code2 size={20} />, color: "text-cyan-400" },
              { label: "Technologies", value: "10+", icon: <Cpu size={20} />, color: "text-blue-400" },
              { label: "Commitment", value: "100%", icon: <Sparkles size={20} />, color: "text-green-400" },
            ].map((stat, idx) => (
              <div key={idx} className="p-6 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:border-purple-500/30 transition-all duration-300 hover:scale-105 group">
                <div className="flex items-center gap-3 mb-3">
                  <div className={`p-2 rounded-lg bg-gradient-to-r from-purple-500/10 to-cyan-500/10 ${stat.color}`}>{stat.icon}</div>
                  <span className={`text-3xl font-bold ${stat.color}`}>{stat.value}</span>
                </div>
                <p className="text-sm text-muted-foreground group-hover:text-white transition-colors">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
            ...
            <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
              <div className="max-w-xl lg:max-w-3xl text-center lg:text-left">
                <div className="inline-flex items-center gap-2 mb-4">
                  ...
                  <span className="text-sm font-semibold uppercase tracking-widest text-cyan-400">Ready to collaborate?</span>
                </div>
                <h2 className="text-3xl md:text-4xl font-extrabold mb-4">
                  <span className="bg-gradient-to-r from-purple-400 to-cyan-400 bg-clip-text text-transparent">Let's Build Something Exceptional Together</span>
                </h2>
                <p className="text-lg text-muted-foreground/90">I'm currently available for frontend development projects and new opportunities.</p>
              </div>
              <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto justify-center lg:justify-end">
                <a href="#contact" className="group relative inline-flex items-center justify-center px-8 py-3 w-full sm:w-auto bg-gradient-to-r from-purple-600 to-blue-600 text-white font-bold rounded-xl shadow-lg shadow-purple-500/40 overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-purple-400/60">
                  <span className="relative z-10 flex items-center gap-2"><Mail size={20} />Hire Me / Contact</span>
                ...
                <a href="#projects" className="group inline-flex items-center justify-center px-6 py-3 w-full sm:w-auto bg-white/5 border border-cyan-500/50 text-cyan-400 font-semibold rounded-xl transition-all duration-300 hover:bg-cyan-500/10 hover:text-white">
                  <span className="flex items-center gap-2"><Code size={18} />View Projects<ArrowRight size={18} className="transition-transform group-hover:translate-x-1" /></span>
                </a>
              </div>
            </div>
          </div>
